...
The system SHALL populate the all map with the full active takeoff catalog and the full active weather-station catalog, using the all-location cache for takeoff data when available and enriching stations with their latest readings before marker creation.
...
#### Scenario: Cached all-location data avoids an immediate refetch
- **GIVEN** the client-side cache contains a valid all-location dataset
- **WHEN** the all-takeoff map loads
- **THEN** the map SHALL use the cached takeoff data to build paragliding and landing markers
- **AND** it SHALL skip the Supabase refetch for that initial load

#### Scenario: Uncached all-location data is fetched through pagination
- **GIVEN** the client-side cache does not contain a valid all-location dataset
- **WHEN** the all-takeoff map loads
- **THEN** the map SHALL fetch all active takeoff rows through the paginated all-location query path
- **AND** it SHALL store that dataset in the all-location cache for reuse
