...
After a successful import from the standalone contribute page, the system SHALL attempt to add the returned location into the client-side paragliding-location cache when that cache already exists.

If the cache update attempt fails, the system SHALL clear the cached paragliding datasets instead of leaving stale cache entries in place.
...
#### Scenario: Import succeeds while cache storage is available
- **GIVEN** the contribute page has successfully received a saved location from the synchronize route
- **WHEN** the browser already has paragliding-location cache data
- **THEN** the system SHALL try to add or update the imported location in the existing cache
- **AND** the system SHALL navigate to the imported location page

#### Scenario: Import succeeds but cache mutation fails
- **GIVEN** the contribute page has successfully received a saved location from the synchronize route
- **WHEN** the cache add-or-update step throws an error
- **THEN** the system SHALL clear the paragliding-location cache
