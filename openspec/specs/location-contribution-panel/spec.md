...
- **AND** the client updates or clears its cached location data before closing the editor and showing a success message
...
The synchronization route SHALL upsert the location by `flightlog_id`. That upsert SHALL preserve existing landing fields and `is_main` state by excluding those fields from the upsert payload. On success, the client SHALL attempt to update cached location data, clear cache if the targeted update fails, and reload the current page so the refreshed location record is shown.
...
- **AND** the client updates or clears cached location data and reloads the page after success
...
- **AND** the client updates cache membership so the location is present in the main-location cache
