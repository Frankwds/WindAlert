...
This capability is owned by the homepage entry point in `src/app/page.tsx`, `src/app/HomePageClient.tsx`, and the `src/app/components/GoogleMaps` stack with `variant='main'`. It depends on shared map-state persistence and shared info-window coordination, but the main-map capability decides the homepage dataset scope, enabled controls, marker filtering rules, and live-refresh behavior.
...
#### Scenario: Uncached main takeoffs are fetched with forecast data
- **GIVEN** the client-side cache does not contain a valid main-takeoff dataset
- **WHEN** the homepage map loads
- **THEN** the map SHALL fetch active `is_main` takeoffs with future forecast rows from Supabase
- **AND** it SHALL store that dataset in the main takeoff cache for reuse
