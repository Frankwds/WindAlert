...
const LandingWeatherTable: React.FC<LandingWeatherTableProps> = ({ forecast, timezone }) => {
  ...
  const { groupedByDay, activeDay, setActiveDay, scrollContainerRef } = useDataGrouping({
  ...
  const hasLandingWind = forecast.some(
    f => f.landing_wind !== null && f.landing_wind !== undefined && f.landing_wind !== 0
  );
  if (!hasLandingWind) {
    return (
      <div className='bg-[var(--background)] rounded-lg'>
        <div className='text-center py-8'>
          <div className='text-[var(--foreground)] mb-4'>Ingen værmelding tilgjengelig enda.</div>
        </div>
      </div>
    );
  }
            ...
            <thead>
              <tr className='border-b border-[var(--border)]'>
                {groupedByDay[activeDay].map((hour, colIndex) => (
                  <th
                    key={colIndex}
                    className='px-1 py-1 whitespace-nowrap bg-[var(--background)] text-[var(--foreground)]'
                  >
                ...
                <tr key={rowIndex} className={`border-b border-[var(--border)] last:border-b-0`}>
                  {groupedByDay[activeDay].map((hour, colIndex) => (
                    <td key={colIndex} className='px-1 py-1 whitespace-nowrap bg-[var(--background)]'>
                      <div className='w-12 flex items-center justify-center mx-auto'>{row.getValue(hour)}</div>
                    </td>
                  ))}
                </tr>
              ))}
