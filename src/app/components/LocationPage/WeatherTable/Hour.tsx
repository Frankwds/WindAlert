...
const Hour: React.FC<HourProps> = ({
  ...
  return (
    <div
      ref={hourRef}
      className={`space-y-1 ${!showValidation && !isExpanded ? 'relative after:absolute after:bottom-0 after:left-2.5 after:right-2.5 after:h-px after:bg-[var(--border)] last:after:hidden' : ''}`}
    >
      <div
        onClick={onToggle}
      ...
      >
        ...
        <div className='flex justify-center'>
          {weatherIcon && <Image src={weatherIcon.image} alt={weatherIcon.description} width={32} height={32} />}
        </div>
        ...
        <div className='text-xs text-blue-500 text-center'>
          {hour.precipitation_max !== 0 ? `${hour.precipitation_min} - ${hour.precipitation_max}` : ''}
        </div>
