...
export function BackToMapButton({ latitude, longitude }: BackToMapButtonProps) {
  ...
  return (
    <Link
      ...
      className="flex flex-1 items-center max-w-18 justify-center gap-1 rounded-lg border border-[var(--border)] bg-[var(--background)] hover:bg-[var(--border)] hover:shadow-[var(--shadow-hover)] cursor-pointer"
      title="Find on Map"
    >
...
export function WindyButton({ latitude, longitude }: WindyButtonProps) {
  return (
    <Link
      ...
      className="flex items-center justify-center max-w-24 gap-1 px-2 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)]  hover:bg-[var(--border)] hover:shadow-[var(--shadow-hover)]  cursor-pointer flex-1 min-w-0"
...
export function YrButton({ latitude, longitude }: YrButtonProps) {
  return (
    <Link
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="flex flex-1 items-center max-w-20 justify-center gap-1 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)]  hover:bg-[var(--border)] cursor-pointer "
      title="View on Yr.no"
    >
...
interface CardFlightlogButtonProps {
  flightlogId: string;
}

export function CardFlightlogButton({ flightlogId }: CardFlightlogButtonProps) {
  return (
    <Link
      href={`https://www.flightlog.org/fl.html?l=2&a=22&country_id=160&start_id=${flightlogId}`}
      target="_blank"
      rel="noopener noreferrer"
      className="flex items-center justify-center max-w-30 gap-1 py-2 px-5 rounded-lg border border-[var(--border)] bg-[var(--background)] hover:bg-[var(--border)] hover:shadow-[var(--shadow-hover)]  cursor-pointer flex-1 min-w-0"
...
export function GoogleMapsButton({ latitude, longitude }: GoogleMapsButtonProps) {
  ...
  return (
    <Link
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="flex flex-1 items-center max-w-30 justify-center gap-1 px-2 py-2 rounded-lg border border-[var(--border)] bg-[var(--background)] hover:bg-[var(--border)] hover:shadow-[var(--shadow-hover)] cursor-pointer flex-1 min-w-0"
